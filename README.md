...
+ Be sure to update to the latest firmware for your camera as Instar have made a lot of improvements recently including adding MQTT features.
...
If you use auto discovery or manually add a camera with PaperUI, it may be required to delete the camera and re-add it for the Openhab DB to be refreshed with the correct data if the binding changes greatly between versions. With auto discovery the camera when found a second time should get the same name due to using the IP address for the unique ID by default.
